		...
		this.each(function(i, cont) {
			...
			var newList = {
				...
				init: function() {
					...
					var that = this;
					$(this.container).bind('sync.dragsort', function(event) {
					  $(that.container).find(opts.dragSelector).each(function(index, element) {
					    if (!$(element).css('cursor') != 'pointer') {
					      $(element).css("cursor", "pointer").mousedown(that.grabItem);
					    }
					  });
					});
				},
